/**
 * Tipos de tokens que el analizador matemático puede identificar.
 */
export type TokenType = 'NUMBER' | 'VAR' | 'CONST' | 'FUNC' | 'OP' | 'LPAREN' | 'RPAREN' | 'UNARY_OP';

/**
 * Representación de un componente léxico (Token).
 */
...
const MAPA_FUNCIONES: Record<string, string> = {
  'sin': 'sin',
...
const OPERADORES: Record<string, { precedence: number; associativity: 'left' | 'right' }> = {
...
export class MathParser {
   ...
   */
  private insertarMultiplicacionImplicitaYUnario(tokens: Token[]): Token[] {
    const result: Token[] = [];
  ...
  public evaluate(x: number): number {
    ...
    for (const token of this.rpn) {
      if (token.type === 'NUMBER') {
        ...
        if (stack.length < 1) throw new Error('Error al evaluar operador unitario.');
        const val = stack.pop()!;
        stack.push(-val);
      } else if (token.type === 'OP') {
        if (stack.length < 2) throw new Error('Error de sintaxis: faltan operandos.');
        const b = stack.pop()!;
        const a = stack.pop()!;
        ...
        if (stack.length < 1) throw new Error(`Falta el argumento para la función: "${token.value}"`);
        const val = stack.pop()!;
        ...
        switch (token.value) {
          ...
          case 'log': stack.push(Math.log(val)); break; // ln y log mapean a logaritmo natural
