...
export class CanvasView {
  ...
  constructor(canvas: HTMLCanvasElement, model: PlotterModel) {
    this.canvas = canvas;
   ...
   */
  public toMap(sx: number, sy: number): { x: number; y: number } {
   ...
   */
  private getGridStep(range: number): number {
  ...
  public paint() {
    ...
    // Marcas del Eje X
    for (let x = startX; x <= this.model.xMax; x += stepX) {
      if (Math.abs(x) < 1e-10) continue; // Saltar el origen (0)
      const { sx } = this.toScreen(x, 0);
    ...
    for (let y = startY; y <= this.model.yMax; y += stepY) {
      if (Math.abs(y) < 1e-10) continue; // Saltar el origen (0)
      const { sy } = this.toScreen(0, y);
    ...
    for (const func of this.model.getFunctions()) {
      if (!func.visible || !func.parser) continue;
